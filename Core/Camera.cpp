...
void BaseCamera::SetViewMatrix(const Matrix4& newViewMatrix)
{
	m_CameraToWorld = ~OrthogonalTransform(Matrix3(newViewMatrix), Vector3(newViewMatrix.GetW()));
}
...
void Camera::Animate(double time)
{
	//path->animate(time);
	if (path)
	{
		glm::vec3 pos, target, up;
		path->animate(time, pos, target, up);
		SetEyeAtUp(Vector3(pos.x, pos.y, pos.z), Vector3(target.x, target.y, target.z), Vector3(up.x, up.y, up.z));
	}
}
