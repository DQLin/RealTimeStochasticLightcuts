...
void ColorBuffer::CreateDerivedViews1D(ID3D12Device* Device, DXGI_FORMAT Format, uint32_t ArraySize, uint32_t NumMips /*= 1*/)
{
	ASSERT(NumMips == 1 && m_FragmentCount == 1, "We don't support mipmaps or MSAA on 1D array");
	m_NumMipMaps = NumMips - 1;
	D3D12_RENDER_TARGET_VIEW_DESC RTVDesc = {};
	D3D12_UNORDERED_ACCESS_VIEW_DESC UAVDesc = {};
	D3D12_SHADER_RESOURCE_VIEW_DESC SRVDesc = {};
	...
	if (ArraySize > 1)
	{
		...
		RTVDesc.Texture1DArray.ArraySize = (UINT)ArraySize;

		UAVDesc.ViewDimension = D3D12_UAV_DIMENSION_TEXTURE1DARRAY;
		UAVDesc.Texture1DArray.MipSlice = 0;
		UAVDesc.Texture1DArray.FirstArraySlice = 0;
		UAVDesc.Texture1DArray.ArraySize = (UINT)ArraySize;
...
void ColorBuffer::CreateTex1D(const std::wstring& Name, uint32_t Width, DXGI_FORMAT Format, D3D12_GPU_VIRTUAL_ADDRESS VidMem /*= D3D12_GPU_VIRTUAL_ADDRESS_UNKNOWN*/)
{
	D3D12_RESOURCE_FLAGS Flags = CombineResourceFlags();
	D3D12_RESOURCE_DESC ResourceDesc = DescribeTex1D(Width, 1, Format, Flags);
	...
	CreateDerivedViews1D(Graphics::g_Device, Format, 1);
}
