...
namespace Graphics
{
	...
	ColorBuffer g_ScenePositionBuffer;
	ColorBuffer g_SceneAlbedoBuffer;
	ColorBuffer g_SceneSpecularBuffer;
	ColorBuffer g_SceneEmissionBuffer;
...
void Graphics::InitializeRenderingBuffers( uint32_t bufferWidth, uint32_t bufferHeight )
{
    ...
    esram.PushStack();
		...
		g_ScenePositionBuffer.Create(L"Position Buffer", bufferWidth, bufferHeight, 1, DXGI_FORMAT_R32G32B32A32_FLOAT, esram);
		g_SceneNormalBuffer.Create(L"Normal Buffer", bufferWidth, bufferHeight, 1, DXGI_FORMAT_R16G16B16A16_FLOAT, esram);
		g_SceneAlbedoBuffer.Create(L"Albedo Buffer", bufferWidth, bufferHeight, 1, DXGI_FORMAT_R8G8B8A8_UNORM, esram);
		g_SceneSpecularBuffer.Create(L"Specular Buffer", bufferWidth, bufferHeight, 1, DXGI_FORMAT_R8G8B8A8_UNORM, esram);
		g_SceneEmissionBuffer.Create(L"Emission Buffer", bufferWidth, bufferHeight, 1, DXGI_FORMAT_R8G8B8A8_UNORM, esram);
        ...
        esram.PushStack();    // Render HDR image
			...
			g_GradLinearDepth.Create(L"Linear Depth Gradient", bufferWidth, bufferHeight, 1, DXGI_FORMAT_R16_UNORM);
