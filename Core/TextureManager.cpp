...
void Texture::CreateFromRawRGBA8Data(const void* data, int imageWidth, int imageHeight, int numComponents, bool sRGB)
{
	if (numComponents == 1) Create(imageWidth, imageHeight, DXGI_FORMAT_R8_UNORM, data);
	else if (numComponents == 2) Create(imageWidth, imageHeight, DXGI_FORMAT_R8G8_UNORM, data);
	else Create(imageWidth, imageHeight, sRGB ? DXGI_FORMAT_R8G8B8A8_UNORM_SRGB : DXGI_FORMAT_R8G8B8A8_UNORM, data);
}
...
ManagedTexture* TextureManager::LoadFromFile( const std::wstring& fileName, bool sRGB )
{
    std::wstring CatPath = fileName;

    ManagedTexture* Tex = LoadDDSFromFile( CatPath + L".dds", sRGB );
    if (!Tex->IsValid())
...
ManagedTexture* TextureManager::LoadDDSFromFile( const std::wstring& fileName, bool sRGB )
{
...
ManagedTexture* TextureManager::LoadTGAFromFile( const std::wstring& fileName, bool sRGB )
{
    auto ManagedTex = FindOrLoadTexture(fileName);
...
ManagedTexture* TextureManager::LoadPIXImageFromFile( const std::wstring& fileName )
{
    auto ManagedTex = FindOrLoadTexture(fileName);
...
ManagedTexture * TextureManager::LoadFromRawData(const std::wstring & fileName, int imageWidth, int imageHeight, int numComponents, const void * data, bool sRGB)
{
	auto ManagedTex = FindOrLoadTexture(fileName);
	ManagedTexture* ManTex = ManagedTex.first;
	const bool RequestsLoad = ManagedTex.second;
	...
	ManTex->CreateFromRawRGBA8Data(data, imageWidth, imageHeight, numComponents, sRGB);
	return ManTex;
}
