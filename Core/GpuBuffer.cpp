...
void GpuBuffer::Create( const std::wstring& name, uint32_t NumElements, uint32_t ElementSize, const void* initialData, D3D12_HEAP_FLAGS heapFlags )
{
    Destroy();
    ...
    ASSERT_SUCCEEDED( 
        g_Device->CreateCommittedResource( &HeapProps, heapFlags,
...
void GpuBuffer::Update(uint32_t startingAddress, uint32_t NumElements, const void* data)
{
	CommandContext::InitializeBuffer(*this, data, NumElements * m_ElementSize, startingAddress);
}
...
void MixedBuffer::CreateDerivedViews(void)
{
	D3D12_SHADER_RESOURCE_VIEW_DESC SRVDesc = {};
	SRVDesc.ViewDimension = D3D12_SRV_DIMENSION_BUFFER;
	SRVDesc.Format = DXGI_FORMAT_UNKNOWN;
