...
#include "ApplyBloom2CS.h"
#include "DebugLuminanceHdrCS.h"
#include "DebugLuminanceHdr2CS.h"
#include "DebugLuminanceLdrCS.h"
#include "DebugLuminanceLdr2CS.h"
#include "GenerateHistogramCS.h"
#include "DebugDrawHistogramCS.h"
...
namespace PostEffects
{
    ...
    BoolVar EnableHDR("Post Effects/HDR/Enable", true);
    BoolVar EnableAdaptation("Post Effects/HDR/Adaptive Exposure", true);
    ExpVar MinExposure("Post Effects/HDR/Min Exposure", 1.0f / 64.0f, -8.0f, 0.0f, 0.25f);
    ExpVar MaxExposure("Post Effects/HDR/Max Exposure", 64.0f, 0.0f, 8.0f, 0.25f);
    NumVar TargetLuminance("Post Effects/HDR/Key", 0.08f, 0.01f, 0.99f, 0.01f);
    NumVar AdaptationRate("Post Effects/HDR/Adaptation Rate", 0.05f, 0.01f, 1.0f, 0.01f);
    ExpVar Exposure("Post Effects/HDR/Exposure", 8.0, -8.0f, 8.0f, 0.25f);
    BoolVar DrawHistogram("Post Effects/HDR/Draw Histogram", false);

    BoolVar BloomEnable("Post Effects/Bloom/Enable", false);
    NumVar BloomThreshold("Post Effects/Bloom/Threshold", 1.5f, 0.0f, 8.0f, 0.1f);        // The threshold luminance above which a pixel will start to bloom
    NumVar BloomStrength("Post Effects/Bloom/Strength", 0.1f, 0.0f, 2.0f, 0.05f);        // A modulator controlling how much bloom is added back into the image
    NumVar BloomUpsampleFactor("Post Effects/Bloom/Scatter", 0.65f, 0.0f, 1.0f, 0.05f);    // Controls the "focus" of the blur.  High values spread out more causing a haze.
    BoolVar HighQualityBloom("Post Effects/Bloom/High Quality", true);                    // High quality blurs 5 octaves of bloom; low quality only blurs 3.
