...
private:
    static const uint32_t kHistorySize = 512;
   // static const uint32_t kExtendedHistorySize = 1024;
...
public:
    ...
    void GatherTimes(uint32_t FrameIndex)
    {
		...
		float CPUTime = 1000.0f * (float)SystemTime::TimeBetweenTicks(m_StartTick, m_EndTick);
		float GPUTime = 1000.0f * m_GpuTimer.GetTime();
		if (m_Name == L"Sample SLC") s_SamplingTime.RecordStat(FrameIndex, GPUTime);
		else if (m_Name == L"Find Mesh Cuts") s_CutFindingTime.RecordStat(FrameIndex, GPUTime);
		else if (m_Name == L"Build light tree (CPU)") s_CPUBuildingTime.RecordStat(FrameIndex, CPUTime);

        m_CpuTime.RecordStat(FrameIndex, CPUTime);
        m_GpuTime.RecordStat(FrameIndex, GPUTime);
    ...
    static void UpdateTimes( void )
    {
        ...
        s_TotalCpuTime.RecordStat(FrameIndex, FrameIndex < 2 ? 0 : 1000*Graphics::GetFrameTime());
	...
	static float GetTotalSamplingTime(void) { return s_SamplingTime.GetAvg() + s_CutFindingTime.GetAvg(); }
	static float GetTotalCpuBuildingTime(void) { return s_CPUBuildingTime.GetAvg(); }
...
private:
	...
	static StatHistory s_SamplingTime;
	static StatHistory s_CPUBuildingTime;
	static StatHistory s_CutFindingTime;
...
StatHistory NestedTimingTree::s_SamplingTime;
StatHistory NestedTimingTree::s_CPUBuildingTime;
StatHistory NestedTimingTree::s_CutFindingTime;
...
namespace EngineProfiling
{
    ...
    void DisplayFrameRate( TextContext& Text )
    {
		...
		//float frameTime = NestedTimingTree::GetFrameDelta();
		float frameRate = 1000.f / cpuTime;
		float sampleTime = NestedTimingTree::GetTotalSamplingTime();
		//float cpuBuildingTime = NestedTimingTree::GetTotalCpuBuildingTime();
		
		//Text.DrawFormattedString("CPU %7.3f ms, GPU %7.3f ms, frame time %7.3f ms, %3u FPS\n",
		//	cpuTime, gpuTime, 1000*frameTime, (uint32_t)(frameRate + 0.5f));
		//Text.DrawFormattedString("Frame Time %4.1f ms %d FPS\n", cpuTime, (uint32_t)(frameRate + 0.5f));

		Text.DrawFormattedString("Frame Time %4.3f ms, Sampling Time %4.3f ms\n", cpuTime, sampleTime);
