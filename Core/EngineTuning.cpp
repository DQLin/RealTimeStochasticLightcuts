...
void HandleDigitalButtonPress( GameInput::DigitalInput button, float timeDelta, std::function<void ()> action )
{
	...
	float timeStretch = durationHeld < 2.0f ? 20.0f : 20.0f;
...
void EngineTuning::Update( float frameTime )
{
	...
	if (GameInput::IsFirstPressed(GameInput::kKey_equals))
		sm_showFPS = !sm_showFPS;
...
{
    FILE* settingsFile;
    fopen_s(&settingsFile, "SavedSettings.txt", "wb");
...
{
    FILE* settingsFile;
    fopen_s(&settingsFile, "SavedSettings.txt", "rb");
    if (settingsFile != nullptr)
    {
...
void EngineTuning::Display( GraphicsContext& Context, float x, float y, float w, float h )
{
	...
	if (sm_showFPS) EngineProfiling::DisplayFrameRate(Text);
	...
	EngineProfiling::Display(Text, 800-x, y, w, h);
