...
#include "GenerateMipsLinearCS.h"
#include "GenerateMipsLinearOddCS.h"
#include "GenerateMipsLinearOddXCS.h"
#include "GenerateMipsLinearOddYCS.h"
#include "GenerateMipsGammaCS.h"
#include "GenerateMipsGammaOddCS.h"
#include "GenerateMipsGammaOddXCS.h"
#include "GenerateMipsGammaOddYCS.h"
...
namespace Graphics
{
    ...
    const char* ResolutionLabels[] = { "360x180", "640x360", "960x540", "1280x720", "1600x900", "1920x1080", "2560x1440", "3200x1800", "3840x2160" };
	eResolution TargetResolution = k720p;

	uint32_t g_CustomResolutionX = 0;
	uint32_t g_CustomResolutionY = 0;
    ...
    NumVar g_HDRPaperWhite("Post Effects/Display/Paper White (nits)", 200.0f, 100.0f, 500.0f, 50.0f);
    NumVar g_MaxDisplayLuminance("Post Effects/Display/Peak Brightness (nits)", 1000.0f, 500.0f, 10000.0f, 100.0f);
    const char* HDRModeLabels[] = { "HDR", "SDR", "Side-by-Side" };
    EnumVar HDRDebugMode("Post Effects/Display/HDR Debug Mode", 0, 3, HDRModeLabels);
    ...
    void SetNativeResolution(void)
    {
		...
		if (g_CustomResolutionX > 0 && g_CustomResolutionY > 0)
		{
			NativeWidth = g_CustomResolutionX;
			NativeHeight = g_CustomResolutionY;
		}
		else
		{
    ...
    EnumVar UpsampleFilter("Post Effects/Display/Upsample Filter", kFilterCount - 1, kFilterCount, FilterLabels);
    NumVar BicubicUpsampleWeight("Post Effects/Display/Bicubic Filter Weight", -0.75f, -1.0f, -0.25f, 0.25f);
    NumVar SharpeningSpread("Post Effects/Display/Sharpness Sample Spread", 1.0f, 0.7f, 2.0f, 0.1f);
    NumVar SharpeningRotation("Post Effects/Display/Sharpness Sample Rotation", 45.0f, 0.0f, 90.0f, 15.0f);
    NumVar SharpeningStrength("Post Effects/Display/Sharpness Strength", 0.10f, 0.0f, 1.0f, 0.01f);
    ...
    EnumVar DebugZoom("Post Effects/Display/Magnify Pixels", kDebugZoomOff, kDebugZoomCount, DebugZoomLabels);
}
