...
namespace Math
{
    ...
    public:
		...
		void SetEyeAtUp2(Vector3 eye, Vector3 at, Vector3 up);
		...
		void SetViewMatrix(const Matrix4& newViewMatrix);
    ...
    public:
		...
		void SetObjectPath(std::shared_ptr<ObjectPath>& _path) { path = _path; };
		...
		float m_DistanceMultipler = 1.0;
		float m_AngleMultipler = 1.0;

		bool m_UseCameraAnimation = false;
    ...
    private:
		...
		std::shared_ptr<ObjectPath> path;
    };
