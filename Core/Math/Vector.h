...
namespace Math
{
    ...
    public:
		...
		INLINE bool operator == (const Vector3& v) const { return (float)this->GetX() == (float)v.GetX() 
													&& (float)this->GetY() == (float)v.GetY() && 
														(float)this->GetZ() == (float)v.GetZ(); }
    ...
    public:
		...
		const Scalar& operator[](size_t i) const { switch (i) { case 0: return GetX(); case 1: return GetY();
													case 2: return GetZ(); case 3: return GetW(); default: return 0;} }
        INLINE void SetX( Scalar x ) { m_vec = XMVectorPermute<4,1,2,3>(m_vec, x); }
        INLINE void SetY( Scalar y ) { m_vec = XMVectorPermute<0,5,2,3>(m_vec, y); }
        INLINE void SetZ( Scalar z ) { m_vec = XMVectorPermute<0,1,6,3>(m_vec, z); }
