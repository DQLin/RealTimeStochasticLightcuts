...
public:
	...
	void ResetCounter(ByteAddressBuffer& Buf, uint32_t Value = 0);
...
public:
    ...
    void ExecuteIndirect(CommandSignature& CommandSig, GpuBuffer& ArgumentBuffer, uint64_t ArgumentStartOffset = 0,
        uint32_t MaxCommands = 1, GpuBuffer* CommandCounterBuffer = nullptr, uint64_t CounterOffset = 0);
	ID3D12Device* GetDevice() { return m_OwningManager->GetDevice(); }
...
inline void CommandContext::ResetCounter(ByteAddressBuffer& Buf, uint32_t Value)
{
	FillBuffer(Buf, 0, Value, sizeof(uint32_t));
	TransitionResource(Buf, D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
}
