...
void CameraController::Update( float deltaTime )
{
    ...
    float speedScale = (m_FineMovement ? 0.1f : 1.0f) * timeScale * m_TargetCamera.m_DistanceMultipler;
    float panScale = (m_FineRotation ? 0.5f : 1.0f) * timeScale * m_TargetCamera.m_AngleMultipler;

    float yaw = GameInput::GetAnalogInput(GameInput::kAnalogMouseX) * 0.2 * m_HorizontalLookSensitivity * panScale;
    float pitch = GameInput::GetAnalogInput(GameInput::kAnalogMouseY) * 0.2 * m_VerticalLookSensitivity * panScale;
...
void CameraController::Update(float deltaTime, bool& hasChange)
{
	(deltaTime);
	...
	hasChange = false;
	if (yaw != 0 || pitch != 0 || forward != 0 || strafe != 0 || ascent != 0) hasChange = true;
