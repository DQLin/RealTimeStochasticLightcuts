...
{
    BoolVar EnableTAA("Post Effects/AA/TAA/Enable", false);
    NumVar Sharpness("Post Effects/AA/TAA/Sharpness", 0.5f, 0.0f, 1.0f, 0.25f);
    NumVar TemporalMaxLerp("Post Effects/AA/TAA/Blend Factor", 1.0f, 0.0f, 1.0f, 0.01f);
    ExpVar TemporalSpeedLimit("Post Effects/AA/TAA/Speed Limit", 64.0f, 1.0f, 1024.0f, 1.0f);
    BoolVar TriggerReset("Post Effects/AA/TAA/Reset", false);
	...
	void ApplyTemporalAACustomImage(ComputeContext & Context, ColorBuffer & customImage);
    void SharpenImage(ComputeContext& Context, ColorBuffer& TemporalColor);
	void SharpenCustomImage(ComputeContext & Context, ColorBuffer & TemporalColor, ColorBuffer & customImage);
}
...
void TemporalEffects::ResolveCustomImage(CommandContext& BaseContext, ColorBuffer& customImage, bool EnableCustomTAA)
{
	ScopedTimer _prof(L"Temporal Resolve", BaseContext);
	...
	if (EnableCustomTAA != s_EnableTAA || TriggerReset)
	{
		ClearHistory(Context);
		s_EnableTAA = EnableCustomTAA;
		TriggerReset = false;
	}
	...
	if (EnableCustomTAA)
	{
		ApplyTemporalAACustomImage(Context, customImage);
		SharpenCustomImage(Context, g_TemporalColor[Dst], customImage);
	}
}
...
void TemporalEffects::ApplyTemporalAACustomImage(ComputeContext& Context, ColorBuffer& customImage)
{
	ScopedTimer _prof(L"Resolve Image", Context);
	...
	ConstantBuffer cbv = {
		1.0f / customImage.GetWidth(), 1.0f / customImage.GetHeight(),
		(float)TemporalMaxLerp, 1.0f / TemporalSpeedLimit,
		0, 0
	};
	...
	Context.TransitionResource(g_VelocityBuffer, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);
	Context.TransitionResource(customImage, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);
	Context.TransitionResource(g_TemporalColor[Src], D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);
	Context.TransitionResource(g_TemporalColor[Dst], D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
	Context.TransitionResource(g_LinearDepth[Src], D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);
	Context.TransitionResource(g_LinearDepth[Dst], D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);
	Context.SetDynamicDescriptor(1, 0, g_VelocityBuffer.GetSRV());
	Context.SetDynamicDescriptor(1, 1, customImage.GetSRV());
	Context.SetDynamicDescriptor(1, 2, g_TemporalColor[Src].GetSRV());
	Context.SetDynamicDescriptor(1, 3, g_LinearDepth[Src].GetSRV());
	Context.SetDynamicDescriptor(1, 4, g_LinearDepth[Dst].GetSRV());
	Context.SetDynamicDescriptor(2, 0, g_TemporalColor[Dst].GetUAV());

	Context.Dispatch2D(customImage.GetWidth(), customImage.GetHeight(), 16, 8);
}
...
void TemporalEffects::SharpenCustomImage(ComputeContext& Context, ColorBuffer& TemporalColor, ColorBuffer& customImage)
{
	ScopedTimer _prof(L"Sharpen or Copy Image", Context);

	Context.TransitionResource(customImage, D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
	Context.TransitionResource(TemporalColor, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);

	Context.SetPipelineState(Sharpness >= 0.001f ? s_SharpenTAACS : s_ResolveTAACS);
	Context.SetConstants(0, 1.0f + Sharpness, 0.25f * Sharpness);
	Context.SetDynamicDescriptor(1, 0, TemporalColor.GetSRV());
	Context.SetDynamicDescriptor(2, 0, customImage.GetUAV());
	Context.Dispatch2D(customImage.GetWidth(), customImage.GetHeight());
}
